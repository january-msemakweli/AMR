#!/usr/bin/env Rscript
# Same as make_amr_its_counterfactual_plots.R but renders one panel: (a),(b),(c) + legend.
# Layout: top-left (a), top-right (b), bottom-left (c), bottom-right (legend).
# Run from the AMR folder (e.g. setwd("AMR") or Rscript AMR/make_amr_its_counterfactual_panel.R from project root).
...
# ---- SCB population helpers (same as make_amr_its_counterfactual_plots.R) ----
...
# Returns ggplot only (no ggsave). drop_legend = TRUE for panel cells.
make_plot_for_pathogen <- function(pathogen_name, drop_legend = FALSE) {
  ...
  p <- ggplot(nat, aes(x = year)) +
    ...
    scale_x_continuous(limits = c(2015, 2025), breaks = 2015:2025) +
    labs(x = "Year", y = "Notification rate per 100,000", color = NULL) +
    theme_gray(base_size = 11, base_family = "serif") +
    theme(
      legend.position = if (drop_legend) "none" else "right",
...
# Build three panels (no legend) with tags (a), (b), (c)
p_a <- make_plot_for_pathogen("ESBL-CARBA", drop_legend = TRUE) + labs(tag = "(a)")
p_b <- make_plot_for_pathogen("VRE",        drop_legend = TRUE) + labs(tag = "(b)")
p_c <- make_plot_for_pathogen("MRSA",      drop_legend = TRUE) + labs(tag = "(c)")

# One plot with legend so we can extract it for bottom-right (slightly larger legend)
p_with_legend <- make_plot_for_pathogen("ESBL-CARBA", drop_legend = FALSE) +
  theme(legend.text = element_text(size = 12.5), legend.key.size = unit(0.9, "cm"))
legend_grob <- cowplot::get_legend(p_with_legend)

# Layout: top-left (a), top-right (b), bottom-left (c), bottom-right = legend
panel <- (p_a + p_b) / (p_c + patchwork::wrap_elements(full = legend_grob))

out_path <- file.path("amr_its_plots", "its_panel_three_plots.png")
ggsave(out_path, panel, width = 11, height = 8, dpi = 600)

cat("Done. Wrote panel to amr_its_plots/its_panel_three_plots.png\n")
